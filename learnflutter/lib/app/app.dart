...
/// Lớp AppStrings đóng vai trò là một kho lưu trữ tập trung dành riêng cho các hằng số cấu hình của widget App trong toàn bộ hệ thống.
...
/// Lớp App thực hiện nhiệm vụ quản lý toàn bộ luồng khởi tạo ban đầu và điều phối cơ chế điều hướng chính khi ứng dụng Flutter bắt đầu khởi động.
...
/// Bên cạnh việc hiển thị giao diện, App còn đóng vai trò như một bộ điều phối để chuẩn bị các thông số kích thước màn hình và đồng bộ hóa các tác vụ khởi chạy ngầm.
    ...
    return SafeArea(
      ...
      child: BlocBuilder<SettingThemeCubit, SettingThemeState>(
        builder: (context, state) {
          ...
          /// Do phương thức này được đặt trong hàm build, nó cho phép ứng dụng phản hồi linh hoạt with các thay đổi về hướng xoay hoặc thay đổi kích thước cửa sổ của người dùng.
          ...
          return FlutterSplashScreen.gif(
            backgroundImage: Image.asset(AppStrings.backgroundImage),
            onInit: () async {
              debugPrint(
                  AppLocaleTranslate.splashInitialized.getString(context));
              await Future.delayed(
                const Duration(seconds: AppStrings.splashDurationSeconds),
              );
              initCompleter.complete();
            },
            onEnd: () async {
              debugPrint(
                  AppLocaleTranslate.splashAnimationEnded.getString(context));
              await initCompleter.future;
            },
            nextScreen: const IntroSplash(),
            gifPath: AppStrings.splashGifPath,
