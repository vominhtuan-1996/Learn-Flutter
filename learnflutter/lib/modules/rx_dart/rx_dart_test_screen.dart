...
class RxDartScreenState extends State<RxDartScreen> {
  ...
  Stream<int> firstStream() async* {
    await Future.delayed(const Duration(seconds: 1)); // 1s sau sẽ emit event đầu tiên
    yield 20; // emit 20 vào giây thứ 1
    await Future.delayed(const Duration(seconds: 1)); // nghỉ 1 giây
...
// stream 2
  Stream<int> secondStream() async* {
    await Future.delayed(const Duration(seconds: 7)); // sau 7s sẽ emit event đầu tiên
    yield 1;
