...
abstract class TapBuilderWidget extends StatefulWidget {
  const TapBuilderWidget({
    Key? key,
    required this.onTap,
...
abstract class TapBuilderBaseState<T extends TapBuilderWidget> extends State<T> {
  ...
  Future<void> unpressAfterDelay(
    ...
    if (isPressed && _pressCount == initialPressCount && mounted) {
      setState(() {
        isPressed = false;
      });
    }
  ...
  bool isWidgetEnabled(TapBuilderWidget widget) {
