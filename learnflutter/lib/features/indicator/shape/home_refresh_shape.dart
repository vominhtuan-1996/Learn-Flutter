...
import 'package:learnflutter/features/indicator/paint/home_refresh_painter.dart';

// class HomeRefreshShape extends StatefulWidget {
//   final double progress;
//   final IndicatorState state;
//   final Color color;
//   final Size size; // 👈 thêm size tuỳ chỉnh
//   final double bounce;
//   const HomeRefreshShape({
//     super.key,
...
//   });
//   @override
//   State<HomeRefreshShape> createState() => _HomeRefreshShapeState();
// }

// class _HomeRefreshShapeState extends State<HomeRefreshShape> {
//   bool hasTriggeredHaptic = false;

//   @override
//   void didUpdateWidget(covariant HomeRefreshShape oldWidget) {
...
//           scale: (scale * widget.bounce).clamp(0.0, 1.0),
//           child: CustomPaint(
//             size: widget.size,
//             painter: HomeRefreshPainter(progress: widget.progress),
//           ),
//         ),
//       ),
//     );
...
class _HomeRefreshShapeState extends State<HomeRefreshShape> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _loadingController = AnimationController(
      ...
      duration: const Duration(milliseconds: 1000),
        ...
        if (widget.state == IndicatorState.loading) setState(() {});
      });
  }
  ...
  void didUpdateWidget(covariant HomeRefreshShape oldWidget) {
    ...
    if (widget.state == IndicatorState.loading && !_loadingController.isAnimating) {
      _loadingController.repeat();
    } else if (widget.state != IndicatorState.loading && _loadingController.isAnimating) {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.state == IndicatorState.dragging || widget.state == IndicatorState.settling) {
      scale = lerpDouble(0.6, 1.0, widget.progress)!;
      opacity = widget.progress;
    } else if (widget.state == IndicatorState.armed || widget.state == IndicatorState.loading) {
      scale = 1.0;
      opacity = 1.0;
    } else if (widget.state == IndicatorState.complete) {
      scale = lerpDouble(1.0, 1.4, widget.progress)!;
    ...
    return Opacity(
      ...
      child: Transform.scale(
        ...
        child: CustomPaint(
          size: widget.size,
          painter: HomeRefreshPainter(progress: progress),
        ),
      ),
    );
  }
