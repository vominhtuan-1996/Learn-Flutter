// ignore_for_file: prefer_initializing_formals, unnecessary_null_comparison, library_private_types_in_public_api, curly_braces_in_flow_control_structures, unused_import, constant_identifier_names, sort_child_properties_last
...
class SmartRefresher extends StatefulWidget {
  ...
  const SmartRefresher({
    Key? key,
    required this.child,
    required IndicatorBuilder headerBuilder,
    required IndicatorBuilder footerBuilder,
...
//  bool _isDragging = false;
  ...
  ValueNotifier<double> offsetLis = ValueNotifier(0.0);

  ValueNotifier<int> topModeLis = ValueNotifier(0);

  ValueNotifier<int> bottomModeLis = ValueNotifier(0);
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> slivers =
        List.from(child.buildSlivers(context), growable: true);
...
abstract class Indicator extends StatefulWidget {
  ...
  const Indicator({required Key key, required this.mode}) : super(key: key);
}
